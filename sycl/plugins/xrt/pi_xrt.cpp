...
///
pi_result check_error(pi_result result, const char *function, int line,
                      const char *file) {
  ...
  if (std::getenv("PI_XRT_ABORT") != nullptr) {
    std::abort();
  }
...
pi_result getInfo(size_t param_value_size, void *param_value,
  ...
  auto assignment = [](void *param_value, T value, size_t) {
...
_pi_platform::_pi_platform(unsigned int numDevices):refCount_{1} {
...
pi_result xrt_piPlatformsGet(pi_uint32 num_entries, pi_platform *platforms,
  ...
  static std::optional<_pi_platform> platform;
  ...
  std::call_once(
      initFlag,
      [](pi_result &err) {
        auto numDevices = xrt::system::enumerate_devices();
        if (numDevices) {
          try {
            _pi_platform tmpPlatform(numDevices);
            platform = std::forward<_pi_platform>(tmpPlatform);
  ...
  if (platforms != nullptr && platform.has_value()) {
...
pi_result xrt_piDeviceRetain(pi_device) { return PI_SUCCESS; }

/// \return PI_SUCCESS always since CUDA devices are always root devices.
///
pi_result xrt_piDeviceRelease(pi_device) { return PI_SUCCESS; }
...
pi_result xrt_piDeviceGetInfo(pi_device device, pi_device_info param_name,
  ...
  case PI_DEVICE_INFO_MAX_CLOCK_FREQUENCY: {
    return getInfo(param_value_size, param_value, param_value_size_ret, 
        static_cast<int>(native_dev.get_info<xrt::info::device::max_clock_frequency_mhz>()));
  }
  ...
  case PI_DEVICE_INFO_ERROR_CORRECTION_SUPPORT: {
    // TODO get something device specific
    return getInfo(param_value_size, param_value, param_value_size_ret, PI_TRUE);
  }
  case PI_DEVICE_INFO_HOST_UNIFIED_MEMORY: {
    return getInfo(param_value_size, param_value, param_value_size_ret, PI_FALSE);
  }
  ...
  case PI_DEVICE_INFO_PROFILE: {
    return getInfo(param_value_size, param_value, param_value_size_ret, "EMBEDDED_PROFILE");
  }
...
pi_result xrt_piContextGetInfo(pi_context context, pi_context_info param_name,
  ...
  case PI_CONTEXT_INFO_NUM_DEVICES:
    return getInfo(param_value_size, param_value, param_value_size_ret, context->devices_.size());
  case PI_CONTEXT_INFO_DEVICES:
    return getInfoArray(context->devices_.size(), param_value_size, param_value,
                        param_value_size_ret, context->devices_.data());
...
///
pi_result xrt_piDevicePartition(pi_device,
...
pi_result xrt_piextDeviceGetNativeHandle(pi_device device,
  ...
  *nativeHandle = reinterpret_cast<pi_native_handle>(&(device->get()));
  return PI_SUCCESS;
}
...
_pi_context::_pi_context(pi_uint32 num_devices, const pi_device *devices):refCount_{1}{
    devices_.reserve(num_devices);
    std::copy(devices, devices + num_devices, std::back_inserter(devices_));
}
...
pi_result xrt_piContextCreate(const pi_context_properties *properties,
  ...
  try {
    auto piContextPtr = std::make_unique<_pi_context>(num_devices, devices);    
...
pi_result xrt_piContextRelease(pi_context ctxt) {
...
#define _PI_CL(pi_api, xrt_api)                                               \
  ...
  _PI_CL(piextQueueCreateWithNativeHandle,
         xrt_piextQueueCreateWithNativeHandle)
  // Memory
  ...
  _PI_CL(piextEventCreateWithNativeHandle,
         xrt_piextEventCreateWithNativeHandle)
  // Sampler
